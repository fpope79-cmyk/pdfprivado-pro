...
assert.match(source, /resolveOcrProfile\("precise"\)/);
assert.match(source, /shouldTryPreciseOcr/);
assert.match(source, /chooseBetterOcrRecord/);
assert.match(source, /characterCoverage < 0\.90/);
assert.match(source, /wordCoverage < 0\.84/);
assert.match(source, /inspectRasterContentForHybridOcr/);
assert.match(source, /buildHybridPageContent/);
assert.match(source, /hybridRequested/);
assert.match(source, /balancedRetried/);
assert.match(source, /preciseRetried/);
assert.match(source, /blankSkipped/);
assert.match(source, /balancedOcrPool/);
assert.match(source, /preciseOcrPool/);
assert.match(pool, /MAXIMUM_POOL_SIZE\s*=\s*4/);

console.log("OK  OCR adaptativo: concurrencia, híbrido raster, cobertura y tercera pasada precisa validados");
