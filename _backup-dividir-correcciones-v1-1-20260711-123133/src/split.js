...
function buildCurrentPlan() {
  ...
  updatePlanPreview(result.errors);
}

function updatePlanPreview(errors = []) {
  outputList.replaceChildren();
...
async function chooseDestinationFolder() {
  ...
  if (typeof dialog?.open === "function" && typeof fs?.writeFile === "function") {
    const path = await dialog.open({ directory: true, multiple: false, title: "Elige la carpeta para los PDF divididos" });
    if (!path) return null;
    return { kind: "tauri", path, fs, label: String(path) };
  }
...
async function writeResult(target, name, bytes) {
  ...
  if (target.kind === "tauri") {
    await target.fs.writeFile(pathJoin(target.path, name), bytes);
    return;
  }
...
async function savePlan() {
  ...
  try {
    ...
    for (let index = 0; index < state.plan.length; index += 1) {
      ...
      await writeResult(target, finalName, bytes);
      created += 1;
    ...
    const message = error instanceof Error ? error.message : "No se pudo completar la división.";
    setSaveProgress(0, "No se pudo completar la división", message, "error");
    setFeedback(message, "error");
  } finally {
    state.saving = false;
    state.cancelRequested = false;
...
[rangesInput, everyInput, partsInput, cutsInput, sizeInput].forEach((input) => {
  input?.addEventListener("input", () => buildCurrentPlan());
});
excludeBlankOption?.addEventListener("change", buildCurrentPlan);
buildPlanButton?.addEventListener("click", () => {
  buildCurrentPlan();
  outputList?.scrollIntoView({ behavior: "smooth", block: "nearest" });
});
namePattern?.addEventListener("input", updatePlanPreview);
